...
function stimScreen() {
  if (expType == 5) {
    ...
    stimOnset[trialCount] = performance.now();
...
function itiScreen() {
  ...
  data.push([sectionType, block, blockType,
    ...
    stimDiff[block - 1], incArr[trialCount], switchArr[trialCount], taskName[taskArr[trialCount]], partResp, stimOnset[trialCount],
    respOnset[trialCount], respArr[trialCount]]);
